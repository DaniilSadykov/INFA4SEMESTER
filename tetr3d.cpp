...
public:
    ...
    CalcNode(double x, double y, double z, double smth, double vx, double vy, double vz) 
            : x(x), y(y), z(z), smth(smth), vx(vx), vy(vy), vz(vz), t(t)
    {
    }
    ...
    void move(double tau) {
        ...
        z += vz * cos(t);
        t += tau;
        smth = pow(x, 2)*cos(t) + pow(y, 2)*cos(t) + pow(z, 2)*cos(t);
    }
};
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double vx = pow(pointY - pointZ, 2)/40;
            double vy = pow(pointZ - pointX, 2)/30;
            double vz = pow(pointX - pointY, 2)/50;
...
int main()
{
    ...
    gmsh::initialize();
    gmsh::model::add("untitled");

    // Считаем STL
    try {
        gmsh::merge("untitled.stl");
    } catch(...) {
    ...
    for(unsigned int step = 1; step < 300; step++) {
        mesh.doTimeStep(tau);
        mesh.snapshot(step);
    }
